...
export default function Navbar() {
  ...
  return (
    <>
      <div
        className={`${navbar ? "bg-slate-50 shadow-lg " : " bg-transparent "
     ...
     ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <nav
          ref={navRef}
          className={`w-full bg-white text-blue-900 transition-all duration-300 ease-in-out flex flex-col gap-4 text-center p-4 shadow rounded ${isOpen ? "translate-y-0 scale-100" : "translate-y-[100vh] scale-0"
            }`}
        >
          <button
            onClick={() => setIsOpen(!isOpen)}
