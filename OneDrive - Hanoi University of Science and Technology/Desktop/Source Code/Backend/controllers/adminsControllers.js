...
const adminLogin = async (req, res) => {
  ...
  if (!req.body.email || !req.body.password)
    return res.status(404).send("Vui lòng nhập đầy đủ thông tin");
  ...
  try {
    ...
    if (isCorrectPassword) {
      ...
      return res.status(404).send("Sai thông tin đăng nhập");
    }
  } catch (error) {
    if (!admin || !isCorrectPassword) {
      return res
        .status(404)
        .send("Sai thông tin đăng nhập");
    }
...
const updateAdmin = async (req, res) => {
  try {
    ...
    //get updated admin info & send back
...
const updateOwner = async (req, res) => {
  try {
    ...
    //get updated Owner and send back
...
const updateAdminRole = async (req, res) => {
  //check if new role is the old role
